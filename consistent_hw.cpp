...
int main()
{
  ...
  cout << "List all the edges and their sameness" << endl;
  ...
  for (int eid=1; eid<=num_edges; eid++)
    {
      ...
      E[eid].same = read_in_between(0,1);
      E[eid+1].begin = E[eid].end;
      E[eid+1].end = E[eid].begin;
      E[eid+1].same = E[eid].same;
      u = E[eid].begin;
      V[u].num_neighbors++;
      v = E[eid].end;
      V[v].num_neighbors++;
  ...
  bool success = bfs(V, E, num_vertices, num_edges);

  if (success)
    {
      cout << "It is consistent" << endl;
    }
  else
    {
      cout << "It is not consistent" << endl;
    }
  ...
  while(!empty(Q))
      ...
      else // explored already
	{
	  ...
	  if (V[u].depth == V[v].depth) // same level			
	    {		
		do{
			count = count + E[V[v].in_edge].same + E[V[u].in_edge].same;
	    ...
	    {
		count = count + E[V[v].in_edge].same;
		v = E[V[v].in_edge].begin;		

		do{
			count = count + E[V[u].in_edge].same + E[V[u].in_edge].same;
			u = E[V[u].in_edge].begin;
			v = E[V[v].in_edge].begin;
		}while(u != v);
