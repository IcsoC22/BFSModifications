...
void bfs(Vertex *V, Edge *E, int num_vertices, int num_edges, int s, int t);
...
int main()
{
  ...
  bfs(V, E, num_vertices, num_edges, s, t);
  ...
  while(!empty(Q))
      ...
      else // explored already
	{
	  if (V[u].depth + 1 == V[v].depth) // next level
	    {
		if (V[u].odd_count > V[v].odd_count){
			V[v].in_edge=eid;
			if(v % 2 == 1){
			  V[v].odd_count = V[u].odd_count + 1;
			}
		}
